...
class OrdinalRegressionLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        loss = F.binary_cross_entropy_with_logits(predictions, ordinal_targets)
        return loss

class CombinedLoss(nn.Module):
    def __init__(self, class_weights=None, num_classes=5):
        super().__init__()
        self.focal_loss = FocalLoss(alpha=1, gamma=2, class_weights=class_weights, reduction="mean")
    ...
    def forward(self, cls_preds, reg_preds, targets):
        ...
        ord_loss = self.ordinal_loss(reg_preds, targets)
        total_loss = 0.6 * cls_loss + 0.4 * ord_loss
        
        return total_loss, cls_loss, ord_loss
...
def evaluate_improved(model, val_loader):
    ...
    with torch.no_grad():
        for images, labels in tqdm(val_loader, desc="Evaluating"):
            ...
            # Ensure regression outputs are properly shaped
            if len(reg_outputs.shape) == 1:
                reg_outputs = reg_outputs.unsqueeze(1)
            elif len(reg_outputs.shape) > 2:
                reg_outputs = reg_outputs.squeeze()

            # Regression to class prediction
            reg_preds = torch.clamp(reg_outputs.float(), 0, 4)
            reg_classes = torch.round(reg_preds).long().clamp(0, 4)

            # Convert reg classes to one-hot probability vectors
            batch_size = cls_probs.size(0)
            num_classes = cls_probs.size(1)
            reg_probs = torch.zeros(batch_size, num_classes, device=cls_probs.device)
            
            # Ensure reg_classes has correct shape [batch_size, 1]
            if len(reg_classes.shape) == 1:
                reg_classes = reg_classes.view(-1, 1)
            
            # Safe scatter operation
            try:
                reg_probs.scatter_(1, reg_classes, 1.0)
            except RuntimeError as e:
                print(f"Error in scatter: {e}")
                print(f"reg_probs shape: {reg_probs.shape}")
                print(f"reg_classes shape: {reg_classes.shape}")
                print(f"reg_classes values: {reg_classes}")
                raise
